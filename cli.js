...
const HELP = `jkb — JKB Ops checklist (edits the synced copy of the live app's data)
  ...
  jkb add <project> <section> <task...> [--priority high|normal|low] [--due YYYY-MM-DD|today|tomorrow]
          [--repeat daily|weekdays|weekly|monthly] [--focus]
  ...
  jkb edit <id> [text...] [--priority p] [--due d|none] [--repeat r|none] [--focus|--unfocus]
          [--project p] [--section s]
...
function parse(argv) {
  const opts = {}, args = [];
  const valued = new Set(["priority", "due", "repeat", "project", "section"]);
...
function line(t) {
  const bits = [t.priority === "high" ? "high" : "", t.due ? "due " + t.due : "", t.repeat ? "↻ " + t.repeat : "", t.steps ? "steps " + t.steps : ""].filter(Boolean);
...
function main(argv) {
  ...
  switch (cmd) {
    ...
    case "add": {
      ...
      const r = store.apply(`add "${rest.join(" ")}"`, s => T.addTask(s, { project, section, text: rest.join(" "), priority: opts.priority, due: opts.due, repeat: opts.repeat, focus: !!opts.focus }));
    ...
    case "edit": {
      const [id, ...rest] = args;
      if (!id) throw new Error("Usage: jkb edit <id> [text...] [--priority p] [--due d] [--repeat r] [--focus|--unfocus] [--project p] [--section s]");
      const changes = {};
