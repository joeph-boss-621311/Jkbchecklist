...
// so every rule here mirrors index.html: ids come from state.nextId, completing a
...
function status(state) {
  ...
  return {
    ...
    overdue: open.filter(x => x.t.due && daysBetween(today, x.t.due) < 0).map(taskView),
    due_soon: open.filter(x => x.t.due && daysBetween(today, x.t.due) >= 0 && daysBetween(today, x.t.due) <= 3).map(taskView),
...
function list(state, { project, section, openOnly } = {}) {
  ...
  return projects.map(p => ({
    ...
    sections: (section ? [findSection(p, section)] : p.sections).map(s => ({
      section: s.name,
      tasks: s.tasks.filter(t => !openOnly || !t.done).map(t => taskView({ t, p, s })),
...
function search(state, query) {
  ...
  return allTasks(state)
    .filter(x => (x.t.text + " " + (x.t.subtasks || []).map(z => z.text).join(" ")).toLowerCase().includes(q))
    .map(taskView);
}

// ---------- changes (each returns a plain summary and leaves state ready to push) ----------
function addTask(state, { project, section, text, priority, due, repeat, focus, steps }) {
  validate(state);
  ...
  const t = { id: newId(state, "t"), text: body, done: false, doneAt: null, priority: normPri(priority), due: normDue(due),
    createdAt: Date.now(), repeat: normRepeat(repeat), subtasks: [], minutes: 0 };
  ...
  if (!!t.done !== !!done) {
    ...
    if (done && t.repeat) {
      next = { id: newId(state, "t"), text: t.text, done: false, doneAt: null, priority: t.priority, due: nextDue(t.due || dayStr(), t.repeat),
        createdAt: Date.now(), repeat: t.repeat, subtasks: (t.subtasks || []).map(z => ({ id: newId(state, "u"), text: z.text, done: false })), minutes: 0 };
      t.repeat = null;
...
function addProject(state, { title, emoji }) {
  ...
  const p = { id: "p" + Date.now().toString(36), emoji: String(emoji || "📌").trim().slice(0, 4) || "📌", title: name, sections: [{ id: newId(state, "s"), name: "General", tasks: [] }] };
...
module.exports = {
  REPEATS, dayStr, addDays, validate, allTasks, findTask, findProject, findSection,
