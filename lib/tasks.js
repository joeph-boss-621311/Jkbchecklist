...
function normTime(t) {
  if (t === null || t === undefined || t === "" || t === "none") return null;
  if (/^([01]\d|2[0-3]):[0-5]\d$/.test(String(t))) return String(t);
  throw new Error(`Time must be HH:MM in 24-hour form or "none" (got "${t}")`);
}
...
function addTask(state, { project, section, text, priority, due, time, repeat, focus, steps, tags }) {
  validate(state);
  const body = String(text || "").trim();
  if (!body) throw new Error("Task text is empty.");
  const p = findProject(state, project);
  const s = sectionOrInbox(state, p, section);
  const t = { id: newId(state, "t"), text: body, done: false, doneAt: null, priority: normPri(priority), due: normDue(due), time: normTime(time),
...
function setDone(state, id, done = true) {
  ...
  if (!!t.done !== !!done) {
    ...
    if (done && t.repeat) {
      next = { id: newId(state, "t"), text: t.text, done: false, doneAt: null, priority: t.priority, due: nextDue(t.due || dayStr(), t.repeat), time: t.time || null,
...
function editTask(state, id, changes = {}) {
  ...
  if (changes.time !== undefined) { const v = normTime(changes.time); if (v !== (t.time || null)) { t.time = v; changed.push("time"); } }
